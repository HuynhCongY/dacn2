...
def show_add_event_page():
    ...
    st.subheader(" Nhập bằng tiếng Việt")
    ...
    if st.button("🚀 Thêm sự kiện từ văn bản", type="primary"):
        if user_input.strip():
            try:
                ...
                with col1:
                    st.write(f"**Tên sự kiện:** {result['event']}")
    ...
    with st.form("manual_event_form"):
        ...
        with col1:
            event = st.text_input("Tên sự kiện *", placeholder="Ví dụ: Họ p nhóm")
            
            start_date = st.date_input("Địa điểm *", value=datetime.now())
        ...
        if submitted:
            if event and start_date and start_time:
                try:
                    # Tạo datetime
...
def show_event_card(event, key_prefix=""):
    ...
    # Kiểm tra xem đang sửa sự kiện này không
    if st.session_state.get('editing_event') == event['id']:
        # Hiển thị form sửa
        with st.form(key=f"edit_form_{event['id']}"):
            st.subheader("✏️ Sửa sự kiện")
            
            col1, col2 = st.columns(2)
            
            with col1:
                event_name = st.text_input("Tên sự kiện *", value=event['event'])
                start_date = st.date_input("Ngày bắt đầu *", value=event['start_time'].date())
                start_time = st.time_input("Giờ bắt đầu *", value=event['start_time'].time())
                location = st.text_input("Địa điểm", value=event.get('location') or "")
            
            with col2:
                end_date = st.date_input("Ngày kết thúc", value=event['end_time'].date() if event.get('end_time') else None)
                end_time = st.time_input("Giờ kết thúc", value=event['end_time'].time() if event.get('end_time') else None)
                reminder_minutes = st.number_input("Nhắc trước (phút)", min_value=0, value=event.get('reminder_minutes') or 0, step=5)
            ...
            with col_cancel:
                cancelled = st.form_submit_button("❌ Hủy")
            ...
            if submitted:
                if event_name and start_date and start_time:
                    try:
                        ...
                        if st.session_state.db.update_event(event['id'], update_data):
...
def show_settings_page():
    ...
    # Checkbox xác nhận
    confirm = st.checkbox("Tôi hiểu và muốn xóa toàn bộ dữ liệu")
    
    if st.button("🗑️ Xóa toàn bộ sự kiện", type="secondary", disabled=not confirm):
...
def main():
    ...
    page = st.sidebar.radio(
        "Chọn trang:",
        ["📅 Thêm sự kiện", "📆 Xem lịch", "⚙️ Xuất/Nhập"]
    )
    ...
    st.sidebar.divider()
    st.sidebar.caption("Ứng dụng quản lý lịch trình cá nhân 2025")
